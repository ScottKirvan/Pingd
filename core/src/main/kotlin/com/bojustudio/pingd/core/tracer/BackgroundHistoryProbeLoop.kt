 ...
 * ping/ping/fake sequencing. `PingdStatusService` uses one of these to keep the settings
 ...
 * `notes/dev/pingd-status-indicator-spec.md`'s "In-App History Graphs": an out-of-scope period
 * is exactly the kind of outage a connectivity history exists to show, not a reason for it to go
 * blind.
 *
 * Deliberately *not* built on [ProbeCycleRunner]. That class's failure retries are floored at
 * a small *fixed* delay ([ProbeCycleRunner.FAILURE_RETRY_DELAY_MS]) for a live tracer outage
 * (see its own class doc) -- but that floor is sized for the case this loop exists to cover
 * only incidentally: a moment of near-instant failures (e.g. DNS resolution) during an
 * otherwise-in-scope network's reconnect. It does not cover the case this loop actually
 * exists for: total connectivity loss (e.g. airplane mode) for as long as `DISABLED`
 * persists, where a connect attempt fails almost instantly because there is no interface to
 * even try, and where the *user's own* pacing preference -- not a small fixed floor -- is
 * otherwise what would govern retries were this built on [ProbeCycleRunner]. An unthrottled
 * (or user-set-to-0 "free wheeling") retry loop against that would spin as fast as the CPU
 * allows for as long as the outage lasts -- directly working against the app's own
 * battery-conscious design. This class paces *every* attempt, success or failure alike, by
 * [retryDelayMs] instead -- callers must floor that value themselves (see
...
class BackgroundHistoryProbeLoop(
     ...
     * way, success or failure alike, and by one single caller-configurable value -- unlike
     * [ProbeCycleRunner], which paces a success by the user's own step-delay preference but a
     * failure by its own separate fixed floor instead, per the class doc. */
    private fun runOnce() {
        if (!running) return
