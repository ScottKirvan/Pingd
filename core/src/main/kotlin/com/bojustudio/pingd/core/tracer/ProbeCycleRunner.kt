 ...
 * queue/executor that is running [start] (see `PingdStatusService.stopCycle`'s comment for
 ...
 * as a no-op (see `PingdStatusService.applyVisibility`'s `ENABLED` branch) still has a way to
...
class ProbeCycleRunner(
    ...
    companion object {
         ...
         * `notes/dev/pingd-status-indicator-spec.md`'s "In-App History Graphs" section cites
