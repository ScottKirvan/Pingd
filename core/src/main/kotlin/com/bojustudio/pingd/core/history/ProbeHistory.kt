 ...
 * (see `PingdProbeHistory`) exactly the way `PingdIconDisplay` holds a drawable id. Every
 ...
 * switched off (the master toggle) — `PingdStatusService` keeps a throttled probe running even
 ...
 * [com.bojustudio.pingd.core.tracer.AckTracer] has none: per spec the sample history is
...
data class ProbeHistory(
     ...
     *   `PingdPreferences.STEP_DELAY_RANGE_MS`), so it is *ordinary*, not rare, for two
    ...
    companion object {
         ...
         * `PingdPreferences.HISTORY_WINDOW_RANGE_MS`'s lower bound in `:app` (duplicated here,
         ...
         * `PingdPreferences.STEP_DELAY_RANGE_MS`'s upper bound (1_000ms) = 2_000ms — see
         ...
         * `PingdPreferences.STEP_DELAY_RANGE_MS`) without an unbounded number of levels. */
