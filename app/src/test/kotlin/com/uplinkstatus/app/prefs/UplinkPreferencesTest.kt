...
/**
 * [UplinkPreferences.stepDelayMs]'s and [UplinkPreferences.historyWindowMs]'s range validation
 * -- everything else on this data class is a plain default, exercised indirectly through
 * [UplinkPreferencesRepositoryTest] and the various fakes/tests that construct it. This file
...
class UplinkPreferencesTest {
    ...
    fun `the default history window is the spec's seven minutes, and within the valid range`() {
        ...
        require(preferences.historyWindowMs in UplinkPreferences.HISTORY_WINDOW_RANGE_MS)
    }

    @Test(expected = IllegalArgumentException::class)
    fun `a history window below the slider's range is rejected`() {
        UplinkPreferences(historyWindowMs = UplinkPreferences.HISTORY_WINDOW_RANGE_MS.first - 1)
    }

    @Test(expected = IllegalArgumentException::class)
    fun `a history window above the slider's range is rejected`() {
        UplinkPreferences(historyWindowMs = UplinkPreferences.HISTORY_WINDOW_RANGE_MS.last + 1)
    }

    @Test
    fun `the history window range's own endpoints are accepted`() {
        UplinkPreferences(historyWindowMs = UplinkPreferences.HISTORY_WINDOW_RANGE_MS.first)
        UplinkPreferences(historyWindowMs = UplinkPreferences.HISTORY_WINDOW_RANGE_MS.last)
    }

    @Test
    fun `the history window range is whole minutes, so the slider's stops land on real values`() {
        assertEquals(0L, UplinkPreferences.HISTORY_WINDOW_RANGE_MS.first % UplinkPreferences.HISTORY_WINDOW_STEP_MS)
        assertEquals(0L, UplinkPreferences.HISTORY_WINDOW_RANGE_MS.last % UplinkPreferences.HISTORY_WINDOW_STEP_MS)
        assertEquals(0L, UplinkPreferences().historyWindowMs % UplinkPreferences.HISTORY_WINDOW_STEP_MS)
    }
    ...
    fun `the widest history window still fits inside the sample cap at default pacing`() {
        ...
        val widestWindowSeconds = UplinkPreferences.HISTORY_WINDOW_RANGE_MS.last / 1000
