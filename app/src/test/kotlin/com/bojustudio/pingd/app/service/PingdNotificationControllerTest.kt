...
/**
 * Verifies [PingdNotificationController]'s reaction to [CycleEvent]s and its
...
@Config(sdk = [34])
class PingdNotificationControllerTest {

    private val context: Context = RuntimeEnvironment.getApplication()
    private val notificationManager: NotificationManager =
        checkNotNull(context.getSystemService(NotificationManager::class.java))
    private val controller = PingdNotificationController(context)

    @Before
    fun grantNotificationsPermission() {
        // PingdNotificationController checks POST_NOTIFICATIONS before calling notify()
     ...
     * real cycle event (below) or a real visibility decision ([PingdStatusServiceTest]).
     */
    @Test
    ...
    fun `an ack reports connected with the measured latency`() {
        ...
        assertEquals(
            PingdActivityStatus.Activity.Connected(latencyMs = 23),
            PingdActivityStatus.activity.value,
        )
    }
    ...
    fun `a freeze reports connection trouble carrying the reason that caused it`() {
        controller.onEvent(CycleEvent.Frozen(BarPosition.BAR_2, FreezeReason.PROBE_FAILURE))
        assertEquals(
            PingdActivityStatus.Activity.ConnectionTrouble(FreezeReason.PROBE_FAILURE),
            PingdActivityStatus.activity.value,
        )

        controller.onEvent(CycleEvent.Frozen(BarPosition.BAR_2, FreezeReason.DNS_RESOLUTION_FAILURE))
        assertEquals(
            PingdActivityStatus.Activity.ConnectionTrouble(FreezeReason.DNS_RESOLUTION_FAILURE),
            PingdActivityStatus.activity.value,
        )
    }
    ...
    fun `an ack still reports connected when POST_NOTIFICATIONS has been revoked`() {
        ...
        assertEquals(
            PingdActivityStatus.Activity.Connected(latencyMs = 11),
            PingdActivityStatus.activity.value,
        )
    }

    // --- PingdIconDisplay mirrors exactly what icon this class just built, unconditionally --
     ...
     * purpose (see [PingdNotificationController.buildNotification]'s doc), and a reader
     ...
     */
    @Test
    fun `every notification builder mirrors its icon to PingdIconDisplay, even the starting placeholder`() {
        controller.notificationForStarting()
        ...
        assertEquals(R.drawable.ic_scan_5, PingdIconDisplay.iconRes.value)
    ...
    fun `an ack mirrors the new bar position`() {
        ...
        assertEquals(R.drawable.ic_scan_4, PingdIconDisplay.iconRes.value)
    }
    ...
    fun `a repeated freeze with the same reason is suppressed -- and does not re-mirror either`() {
        ...
        assertEquals(R.drawable.ic_scan_1, PingdIconDisplay.iconRes.value)
    }
    ...
    fun `an ack still mirrors the icon when POST_NOTIFICATIONS has been revoked`() {
        // Same reasoning as the analogous PingdActivityStatus test above: buildNotification()
    ...
    // --- PingdProbeHistory records real probe attempts, and only those ----------------------
    ...
    fun `the automatic ack contributes no sample at all -- it is not a probe attempt`() {
        ...
        assertNull(PingdProbeHistory.history.value.averageLatencyMs)
    }
     ...
     * a latency is attached to it. [com.bojustudio.pingd.core.tracer.ProbeCycleRunner] does not
    ...
    fun `a dns-resolution freeze is a failed probe attempt too`() {
        ...
        assertEquals(0f, PingdProbeHistory.history.value.successPercent!!, 0.001f)
    }
    ...
    fun `every immediate-retry failure is recorded, even the ones whose notification is suppressed`() {
        ...
        assertEquals(5, PingdProbeHistory.history.value.attemptCount)
