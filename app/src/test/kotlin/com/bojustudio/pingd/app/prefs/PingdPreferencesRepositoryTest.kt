...
/**
 * Verifies [DataStorePingdPreferencesRepository] actually round-trips through a real
 ...
 * [DataStorePingdPreferencesRepository] instance around the same on-disk file where it
...
@Config(sdk = [34])
class PingdPreferencesRepositoryTest {
    ...
    fun `a fresh DataStore reads back documented defaults`() = runTest {
        val repository = DataStorePingdPreferencesRepository(newDataStore())
    ...
    fun `setStepDelayMs coerces a value outside the slider range instead of storing it verbatim`() = runTest {
        ...
        assertEquals(PingdPreferences.STEP_DELAY_RANGE_MS.first, repository.preferencesFlow.first().stepDelayMs)

        repository.setStepDelayMs(5_000L)
        assertEquals(PingdPreferences.STEP_DELAY_RANGE_MS.last, repository.preferencesFlow.first().stepDelayMs)
    }
    ...
    fun `a persisted step delay outside the slider range is coerced back into range on read`() = runTest {
        ...
        val preferences = DataStorePingdPreferencesRepository(dataStore).preferencesFlow.first()

        assertEquals(PingdPreferences.STEP_DELAY_RANGE_MS.last, preferences.stepDelayMs)
    }
    ...
    fun `setHistoryWindowMs coerces a value outside the slider range instead of storing it verbatim`() = runTest {
        ...
        assertEquals(
            PingdPreferences.HISTORY_WINDOW_RANGE_MS.first,
            repository.preferencesFlow.first().historyWindowMs,
        )

        repository.setHistoryWindowMs(24 * 60 * 60_000L)
        assertEquals(
            PingdPreferences.HISTORY_WINDOW_RANGE_MS.last,
            repository.preferencesFlow.first().historyWindowMs,
        )
    }

    /**
     * Coercion on *read* matters more here than for most preferences: [PingdPreferences]'s own
    ...
    fun `a persisted history window outside the slider range is coerced back into range on read`() = runTest {
        ...
        assertEquals(PingdPreferences.HISTORY_WINDOW_RANGE_MS.first, preferences.historyWindowMs)
    }
    ...
    fun `preferences survive a fresh repository instance over the same underlying file`() = runTest {
        ...
        val firstInstance = DataStorePingdPreferencesRepository(dataStore)
        ...
        val secondInstance = DataStorePingdPreferencesRepository(dataStore)
