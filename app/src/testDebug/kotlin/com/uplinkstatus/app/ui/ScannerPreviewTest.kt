...
/**
 * [ScannerPreview] in isolation, driving [UplinkIconDisplay] directly rather than through a
 ...
 * reacts to [UplinkIconDisplay] correctly, live, and renders nothing at all while it's `null`.
 */
...
class ScannerPreviewTest {
    ...
    @Before
    fun reset() = UplinkIconDisplay.resetForTest()

    @After
    fun tearDown() = UplinkIconDisplay.resetForTest()
    ...
    @Test
    fun `renders nothing while UplinkIconDisplay is null`() {
        setContent()
    ...
    @Test
    fun `appears the moment UplinkIconDisplay reports a frame`() {
        setContent()
        composeTestRule.onNodeWithTag(TAG_SCANNER_PREVIEW).assertDoesNotExist()
    ...
    @Test
    fun `disappears again the moment UplinkIconDisplay goes back to null`() {
        UplinkIconDisplay.report(R.drawable.ic_scan_disabled)
        setContent()
        composeTestRule.onNodeWithTag(TAG_SCANNER_PREVIEW).assertExists()
