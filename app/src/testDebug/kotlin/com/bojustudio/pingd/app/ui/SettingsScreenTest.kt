...
import com.bojustudio.pingd.app.BuildConfig
...
import com.bojustudio.pingd.app.prefs.PingdPreferences
import com.bojustudio.pingd.app.prefs.PingdPreferencesRepository
import com.bojustudio.pingd.app.service.PingdStatusService
 ...
 * [FakePingdPreferencesRepository], not just by asserting a click landed.
 *
...
class SettingsScreenTest {
    ...
    private lateinit var repository: FakePingdPreferencesRepository

    @Before
    fun setUp() {
        repository = FakePingdPreferencesRepository()
        ...
        // PingdRuntimeStatus is a process-wide singleton -- without this, a previous test's
        // sequence number (and whatever it last reported) would leak into this one.
        PingdRuntimeStatus.resetForTest()
        // Same reasoning as PingdRuntimeStatus above -- a previous test's last status text
    ...
    /** No real [com.bojustudio.pingd.app.service.PingdStatusService] runs in these tests, so
     * nothing else will ever bump [PingdRuntimeStatus]'s sequence number on its own the way
     ...
     * [PingdVisibility] reported doesn't matter to any of these tests, only the fresh
     * sequence number does. */
    private fun confirmServiceCaughtUp() {
        PingdRuntimeStatus.report(PingdVisibility.ENABLED)
    ...
    fun `turning the master toggle off then back on restarts the service`() {
        // Regression test: PingdStatusService tears itself down completely (stopSelf())
        ...
        while (startedIntent != null) {
            if (startedIntent.component?.className == PingdStatusService::class.java.name) {
                sawServiceRestart = true
            }
            startedIntent = shadowOf(application).nextStartedService
        }
        assertTrue(
            "Expected a startForegroundService() call targeting PingdStatusService after " +
     ...
     *
     * Unlike the test above, this one does not use [FakePingdPreferencesRepository]: its
     * writes are synchronous in-memory assignments, which removes the exact asynchrony the bug
     * lives in. Here the screen talks to a real [DataStorePingdPreferencesRepository] over a
    ...
    fun `re-enabling the master toggle restarts the service only after the new value is persisted`() {
        ...
        val gatedRepository = GatedMasterToggleRepository(DataStorePingdPreferencesRepository(dataStore))
        composeTestRule.setContent {
            MaterialTheme {
        ...
        composeTestRule.waitUntil(timeoutMillis = 5_000) {
            ...
            restarts.contains(PingdStatusService::class.java.name)
        }
    ...
    private fun persistedMasterToggle(repository: PingdPreferencesRepository): Boolean =
    ...
    private class GatedMasterToggleRepository(
        private val delegate: PingdPreferencesRepository,
    ) : PingdPreferencesRepository by delegate {
     ...
     * to [PingdIconDisplay] in isolation; this only has to prove it's actually mounted on the
    ...
    @Test
    fun `the scanner preview mirrors the icon PingdIconDisplay reports, live`() {
        setContent()
     ...
     * fed by the real [PingdProbeHistory], rather than existing only in their own test.
     */
    @Test
    ...
    fun `the status line renders the service's latest reported activity`() {
        ...
        PingdActivityStatus.report(PingdActivityStatus.Activity.Connected(latencyMs = 42))
        composeTestRule.waitForIdle()
    ...
    fun `starting up and a real paused verdict read as different states`() {
        ...
        PingdActivityStatus.report(PingdActivityStatus.Activity.Paused(NetworkScope.WIFI_ONLY))
        composeTestRule.waitForIdle()
